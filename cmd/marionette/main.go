...
func renderUsersPage(ctx *mb.Context, formState createUserFormState) mf.Node {
	header := mf.UIPageHeader(mf.PageHeaderProps{
...
func themeToggleButton() mf.Node {
	return mf.UIThemeToggleButton(mf.ComponentProps{Variant: "outline", Size: "sm"})
}
...
func renderDashboardPage(ctx *mb.Context) mf.Node {
	...
	content := mf.UIStack(mf.StackProps{Gap: "lg"},
		renderDashboardOverview(ctx),
		renderDashboardCharts(ctx),
		mf.UIGrid(mf.GridProps{MinColumnWidth: "lg", Gap: "lg"},
			mf.UICard(mf.CardProps{}, mf.UIEmptyState(mf.EmptyStateProps{
				Title:       "Pending approvals",
				Description: "No approvals are currently waiting.",
				Icon:        "✓",
			})),
			mf.UICard(mf.CardProps{}, mf.UITable(mf.TableProps{
...
func renderAnalyticsPage(ctx *mb.Context) mf.Node {
	header := mf.UIPageHeader(mf.PageHeaderProps{Title: "Analytics", Description: "A chart-first, two-column analytics layout.", Actions: themeToggleButton()})
	content := mf.UISplit(mf.SplitProps{
		Gap: "lg",
		Main: mf.UIStack(mf.StackProps{Gap: "lg"},
			renderDashboardCharts(ctx),
			mf.UIAlert(mf.AlertProps{Title: "Insight", Description: "Editors are growing steadily month-over-month.", Props: mf.ComponentProps{Variant: "info"}}),
		),
		Aside: mf.UIStack(mf.StackProps{Gap: "md"},
...
func renderSettingsPage(ctx *mb.Context) mf.Node {
	header := mf.UIPageHeader(mf.PageHeaderProps{Title: "Settings", Description: "A form-heavy vertical layout for configuration flows.", Actions: themeToggleButton()})
	content := mf.UIStack(mf.StackProps{Gap: "lg"},
		mf.UICard(mf.CardProps{},
			mf.Form("users/reset",
				...
				mf.DivClass("pt-3", mf.UISubmitButton("Save settings", mf.ComponentProps{Variant: "primary"})),
			),
		),
		mf.UIToast(mf.ToastProps{Title: "Tip", Description: "This page demonstrates form-heavy layout variation.", Props: mf.ComponentProps{Variant: "info"}}),
	)
...
func renderDashboardCharts(ctx *mb.Context) mf.Node {
	...
	return mf.UIGrid(mf.GridProps{MinColumnWidth: "md", Gap: "lg"},
		mf.UIChart(mf.ChartProps{
...
func renderDashboardOverview(ctx *mb.Context) mf.Node {
	...
	return mf.UIGrid(mf.GridProps{Columns: "3"},
...
func statCard(label, value, caption, tone string) mf.Node {
	return mf.UICard(mf.CardProps{},
		mf.UIStack(mf.StackProps{Gap: "sm"},
			mf.DivClass("text-sm font-medium text-base-content/60", mf.Text(label)),
			mf.UIStack(mf.StackProps{Direction: "horizontal", Gap: "md", Align: "end", Justify: "between"},
			...
			mf.DivClass("flex items-center justify-between gap-4",
				...
				mf.DivClass("flex items-center gap-2",
					...
					mf.Form("users/loading/start",
						mf.UISubmitButton("Show loading", mf.ComponentProps{Variant: "ghost", Size: "sm", Disabled: loading}),
					).Target("#users-workspace"),
					mf.Form("users/loading/stop",
						mf.UISubmitButton("Show data", mf.ComponentProps{Variant: "ghost", Size: "sm", Disabled: !loading}),
					).Target("#users-workspace"),
					mf.Form("users/reset",
						mf.UISubmitButton("Reset", mf.ComponentProps{Variant: "secondary", Size: "sm"}),
					).Target("#users-workspace"),
				),
			),
			mf.DivClass("overflow-hidden rounded-box border border-base-300", tableBody),
			mf.UIPagination(mf.PaginationProps{
				Page:       pg.Page,
...
func renderUsersTableBody(users []user, loading bool, sortKey string, pg pagination) mf.Node {
	if loading {
		return mf.UIEmptyState(mf.EmptyStateProps{
			Skeleton: true,
			Rows:     5,
		})
	}
	...
	return mf.TableComponentRow{
		Cells: []mf.Node{
			...
			mf.Form("users/delete/prompt",
				...
				mf.UISubmitButton("Delete", mf.ComponentProps{Variant: "danger", Size: "sm"}),
			).Target("#users-workspace"),
		},
	}
}
		...
		mf.DivClass("card-body",
			...
			mf.Form("users/create",
				...
				mf.DivClass("flex flex-wrap gap-2 pt-2",
					mf.UISubmitButton("Create", mf.ComponentProps{Variant: "primary", Size: "sm"}),
					mf.UIButton("Preview", mf.ComponentProps{Variant: "ghost", Size: "sm", Disabled: true}),
				),
			).Target("#users-workspace"),
		),
	)
	...
	return mf.DivClass("grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]",
		mf.DivClass("card bg-base-100 shadow-sm",
			mf.DivClass("card-body gap-4",
				...
				mf.DivClass("grid gap-3 md:grid-cols-2",
					mf.UISkeleton(mf.SkeletonProps{Rows: 4, Props: mf.ComponentProps{Size: "sm"}}),
					mf.UIEmptyState(mf.EmptyStateProps{
				...
				mf.DivClass("rounded-box border border-base-300 bg-base-100 p-3",
					...
					mf.UIThemeToggleButton(mf.ComponentProps{Variant: "outline", Size: "sm"}),
				),
			),
		),
		mf.DivClass("card bg-base-100 shadow-sm",
...
func renderDeleteModal(ctx *mb.Context) mf.Node {
	...
	return mf.UIModal(mf.ModalProps{
		Title: "Delete user",
		...
		Actions: mf.DivClass("flex w-full justify-end gap-2",
			mf.Form("users/delete/cancel",
				mf.UISubmitButton("Cancel", mf.ComponentProps{Variant: "ghost", Size: "sm"}),
			).Target("#users-workspace"),
