...
func MenuIcon(props ComponentProps) Node {
	return element{Tag: "svg", Attrs: map[string]string{
		"xmlns":   "http://www.w3.org/2000/svg",
		"fill":    "none",
		"viewBox": "0 0 24 24",
		"class":   joinClass("inline-block h-6 w-6 stroke-current", props.Class),
	}, Children: []Node{
		element{Tag: "path", Attrs: map[string]string{
...
func chartConfigJSON(props ChartProps) (string, error) {
	...
	for _, dataset := range props.Datasets {
		...
		} else if colors := compactChartColors(dataset.BackgroundColors); len(colors) > 0 {
		...
		} else if colors := compactChartColors(dataset.BorderColors); len(colors) > 0 {
...
func compactChartColors(colors []string) []string {
	if len(colors) == 0 {
		return nil
	}
