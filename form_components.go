...
func Textarea(first any, rest ...any) Node {
	if name, ok := first.(string); ok {
		...
		return TextComponentarea(name, value, options)
	}
...
func Select(first any, rest ...any) Node {
	if name, ok := first.(string); ok {
		...
		return selectComponent(name, options, props)
	}
