...
- `ThemeToggleButton`: the daisyUI implementation keeps the legacy `onclick="globalThis.mrnToggleTheme()"` hook and visible `🌓 Theme` label so existing shell JavaScript and tests keep working while the root API remains a thin alias.
