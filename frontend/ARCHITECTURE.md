...
- `ThemeToggleButton`: the daisyUI implementation keeps the legacy `onclick="globalThis.mrnToggleTheme()"` hook while exposing the active `System`/`Light`/`Dark` mode label so existing shell JavaScript and tests keep working while the root API remains a thin alias.
