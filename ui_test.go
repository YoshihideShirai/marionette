...
func TestComponentInputWithOptionsRendersDateConstraints(t *testing.T) {
	html, err := InputWithOptionsComponent("start_date", "2030-01-01", InputOptions{
		Type:     "date",
		Min:      "2024-01-01",
		Max:      "2026-12-31",
		Required: true,
...
func TestComponentFormFieldRendersLabelHintAndError(t *testing.T) {
	html, err := FormFieldComponent(
		InputWithOptionsComponent("name", "", InputOptions{Required: true, Props: ComponentProps{Variant: "default", Size: "sm"}}),
		FormFieldProps{
			Label:    "Name",
			Required: true,
...
func TestComponentModalRendersSSRState(t *testing.T) {
	closedHTML, err := ModalComponent(ModalProps{
		Title:   "Delete user",
	...
	openHTML, err := ModalComponent(ModalProps{
		Title:   "Delete user",
		Body:    Text("Confirm deletion"),
...
func TestComponentEmptyStateRendersSkeletonAndCopy(t *testing.T) {
	skeletonHTML, err := EmptyStateComponent(EmptyStateProps{Skeleton: true, Rows: 2}).Render()
	if err != nil {
	...
	emptyHTML, err := EmptyStateComponent(EmptyStateProps{Title: "No users", Description: "Create one first."}).Render()
	if err != nil {
...
func TestComponentPaginationRendersState(t *testing.T) {
	html, err := PaginationComponent(PaginationProps{
		Page:       2,
		TotalPages: 4,
...
func TestFeedbackComponentsShareVariantSizeAndA11y(t *testing.T) {
	toastHTML, err := ToastComponent(ToastProps{
		Title:       "Saved",
	...
	alertHTML, err := AlertComponent(AlertProps{Title: "Failed", Description: "Try again.", Icon: "!", Props: ComponentProps{Variant: "error", Size: "md"}}).Render()
	if err != nil {
	...
	skeletonHTML, err := SkeletonComponent(SkeletonProps{Rows: 2, Props: ComponentProps{Variant: "warning", Size: "lg"}}).Render()
	if err != nil {
		t.Fatalf("skeleton render failed: %v", err)
	}
...
func TestLayoutComponentsRenderClassesAndContent(t *testing.T) {
	...
	splitHTML, err := SplitComponent(SplitProps{
		Main:            Text("Main"),
		Aside:           Text("Aside"),
		AsideWidth:      "lg",
...
func TestSurfaceLayoutComponentsRenderHeadersActionsAndChildren(t *testing.T) {
	headerHTML, err := PageHeaderComponent(PageHeaderProps{
		Title:       "Users",
	...
	cardHTML, err := CardComponent(CardProps{
		Title:       "Card",
		Description: "Summary",
	...
	sectionHTML, err := SectionComponent(SectionProps{Title: "Section", Description: "Details"}, Text("Content")).Render()
	if err != nil {
...
func TestFrontendLayoutComponentsMatchRootOutput(t *testing.T) {
	rootHTML, err := GridComponent(GridProps{Columns: "2", Gap: "sm"}, Text("A"), Text("B")).Render()
	if err != nil {
		t.Fatalf("root grid render failed: %v", err)
	}
	frontendHTML, err := mf.GridComponent(mf.GridProps{Columns: "2", Gap: "sm"}, mf.Text("A"), mf.Text("B")).Render()
	if err != nil {
...
func TestComponentChartRendersConfigAndFallback(t *testing.T) {
	html, err := ChartComponent(ChartProps{
		Type:        ChartTypeBar,
		Title:       "Revenue",
...
func TestFrontendChartComponentMatchesRootOutput(t *testing.T) {
	rootHTML, err := ChartComponent(ChartProps{
		Type:   ChartTypeBar,
		Title:  "Orders",
	...
	frontendHTML, err := mf.ChartComponent(mf.ChartProps{
...
func TestComponentDataFrameChartBuildsChartDatasets(t *testing.T) {
	...
	html, err := DataFrameChartComponent(df, DataFrameChartProps{
		Chart: ChartProps{
			Type:  ChartTypeBar,
			Title: "Growth",
		},
...
func TestComponentDataFrameFromCSVUsesDataFrameGoImports(t *testing.T) {
	...
	node, err := DataFrameFromCSVComponent(reader, TableProps{})
	if err != nil {
...
func TestComponentDataFrameFromTSVDefaultsToTabDelimiter(t *testing.T) {
	reader := strings.NewReader("name\trole\nAiko\tAdmin\n")
	node, err := DataFrameFromTSVComponent(reader, TableProps{})
	if err != nil {
